...
parser.add_argument('--start', default=40,
...
parser.add_argument('--use_GAN', default=False, type=bool)
...
cover_dir = '/media/liang/E8B611F5B611C54A/Users/mansh/to_ubuntu/VOC2012/3'
secret_dir = '/media/liang/E8B611F5B611C54A/Users/mansh/to_ubuntu/VOC2012/4'
...
def visualize_batch(batch, trans=True, nrow=4, save=False, name=None):
    grids = vutils.make_grid(batch.to(device)[:], padding=2, normalize=True, nrow=nrow,
                             range=(batch.min().item(), batch.max().item()))
    if trans:
        grids = np.transpose(grids.detach().cpu().numpy(), (1, 2, 0))
    plt.figure()
    plt.imshow(grids)
    if save:
        path = 'training_images/' + name + '.png'
        plt.savefig(path)
    else:
        plt.show()
...
dataset_cov = datasets.ImageFolder(cover_dir, data_transform)

dataloader_cov = torch.utils.data.DataLoader(dataset_cov, batch_size=batch_size, shuffle=False, num_workers=2)

dataset_sec = datasets.ImageFolder(secret_dir, data_transform)

dataloader_sec = torch.utils.data.DataLoader(dataset_sec, batch_size=batch_size, shuffle=False, num_workers=2)
...
for epoch in range(start_epoch, epochs):
    ...
    for i, data in enumerate(zip(dataloader_cov, dataloader_sec)):
        covers = data[0][0]
        secrets = data[1][0]
        ...
        if i % fre_2_print == 0:
            print(
                'epoch: %d || batch: %d || en_loss: %.4f ,  p_loss: %.4f || dis_loss: %.4f || ssim: %.4f|%.4f || mse: %.4f|%.4f || mssim:%.4f|%.4f '
                % (
                    epoch, i + 1, loss, perceptual_loss, errors, s_ssim.item(), c_ssim.item(), s_mse.item(),
                    c_mse.item(),
                    s_mssim.item(), c_mssim.item()))
        ...
        if i % fre_2_show == 0:
            ...
            if use_vis:
                result_c = viz.images(stego_result * 255, nrow=batch_size // 2, win=result_c,
                                      opts=dict(title="cover vs. stego"))
                result_s = viz.images(reveal_result, nrow=batch_size // 2, win=result_s,
                                      opts=dict(title="secret vs. reveal"))
                a = plt.figure()
